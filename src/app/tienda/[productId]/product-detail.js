...
export default function ProductDetail({ productId }) {
    ...
    try {
      ...
      if (selectedPrescriptionLensId) {
        const existingLens = cart.items.find((item) => (
          item.productId === selectedPrescriptionLensId
          && item.mountFrameProductId === product.id
        ));
        requestedItems.push({
  ...
  const prescriptionLenses = product.lensOptions ?? [];
  ...
  return <main className="detail-page">
    ...
    <div className="detail-grid">
      ...
      <section className="detail-copy">
        ...
        <div className="detail-choice detail-prescription-choice"><div><span>Paso 2</span>{product.requiresPrescription ? <><h2>Prepara tu receta</h2><p>Podrás adjuntar una receta externa o ingresar sus valores durante el checkout. Los datos deben confirmarse antes de pagar.</p></> : product.category === "FRAME" ? <><h2>¿Necesitas cristales con receta?</h2><p>El marco se puede comprar solo. Si eliges cristales, quedan asociados a este marco y la receta será obligatoria antes de pagar.</p>{prescriptionLenses.length > 0 ? <fieldset className="lens-options"><legend>Selecciona una opción para este marco</legend><label className="lens-option"><input checked={!selectedPrescriptionLensId} name="prescriptionLens" onChange={() => setSelectedPrescriptionLensId("")} type="radio" /><span><strong>Solo marco</strong><small>Sin cristales ópticos</small></span></label>{prescriptionLenses.map((lens) => <label className="lens-option" key={lens.id}><input checked={selectedPrescriptionLensId === lens.id} name="prescriptionLens" onChange={() => setSelectedPrescriptionLensId(lens.id)} type="radio" /><span><strong>{lens.name}</strong><small>{formatClp(lens.unitPriceCents)} · Requiere receta{lens.isTestData ? " · Datos de prueba" : ""}</small></span></label>)}</fieldset> : <p className="lens-unavailable">Los cristales estarán disponibles cuando se publiquen opciones y precios definitivos.</p>}</> : <><h2>Este producto no requiere receta</h2><p>Puedes agregarlo directamente al carrito y continuar como invitado o con tu cuenta.</p></>}</div><Icon name={product.requiresPrescription || selectedPrescriptionLens ? "file" : "check"} /></div>
