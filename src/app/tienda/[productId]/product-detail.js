...
function ProductImage({ image, priority = false }) {
  return <Image alt={image.alt} className="detail-photo" fill priority={priority} sizes="(max-width: 960px) 100vw, 55vw" src={image.url} />;
}
  ...
  return <main className="detail-page">
    ...
    <div className="detail-grid">
      <section className="detail-gallery" aria-label={`Galería de ${product.name}`}>
        <div className="detail-thumbnails">{images.length > 0 ? images.map((image, index) => <button aria-label={`Mostrar ${image.alt}`} aria-pressed={index === selectedImage} key={image.url} onClick={() => setSelectedImage(index)} type="button"><ProductImage image={image} /></button>) : ["Vista principal", "Vista lateral", "Detalle"].map((label, index) => <button aria-label={label} aria-pressed={index === 0} key={label} type="button"><ProductArt category={product.category} small /></button>)}</div>
