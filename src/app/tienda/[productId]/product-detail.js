  ...
  return <main className="detail-page">
    ...
    <div className="detail-grid">
      ...
      <section className="detail-copy">
        ...
        <div className="detail-choice detail-prescription-choice"><div><span>Paso 2</span>{product.requiresPrescription ? <><h2>Receta opcional</h2><p>Podrás adjuntar una receta externa o ingresar sus valores durante el checkout, sin bloquear el pago.</p></> : product.category === "FRAME" ? <><h2>¿Necesitas cristales?</h2><p>El marco se puede comprar solo. Si eliges cristales, quedan asociados a este marco y la receta se puede adjuntar durante el checkout.</p>{prescriptionLenses.length > 0 ? <fieldset className="lens-options"><legend>Selecciona una opción para este marco</legend><label className="lens-option"><input checked={!selectedPrescriptionLensId} name="prescriptionLens" onChange={() => setSelectedPrescriptionLensId("")} type="radio" /><span><strong>Solo marco</strong><small>Sin cristales ópticos</small></span></label>{prescriptionLenses.map((lens) => <label className="lens-option" key={lens.id}><input checked={selectedPrescriptionLensId === lens.id} name="prescriptionLens" onChange={() => setSelectedPrescriptionLensId(lens.id)} type="radio" /><span><strong>{lens.name}</strong><small>{formatClp(lens.unitPriceCents)} · Receta opcional{lens.isTestData ? " · Datos de prueba" : ""}</small></span></label>)}</fieldset> : <p className="lens-unavailable">Los cristales estarán disponibles cuando se publiquen opciones y precios definitivos.</p>}</> : <><h2>Este producto no requiere receta</h2><p>Puedes agregarlo directamente al carrito y continuar como invitado o con tu cuenta.</p></>}</div><Icon name={product.requiresPrescription || selectedPrescriptionLens ? "file" : "check"} /></div>
