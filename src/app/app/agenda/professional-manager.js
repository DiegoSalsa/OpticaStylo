"use client";
// Código de la aplicación para professional-manager.

import { useEffect, useState } from "react";
import { readResponse } from "@/components/internal/internal-shell";

// Centralizar la lógica de profesional row para mantener consistente el comportamiento de la aplicación
function ProfessionalRow({ item, onSaved }) {
  const [form, setForm] = useState(item);
  const [saving, setSaving] = useState(false);
  // Crear o registrar save aplicando las reglas de negocio y persistencia correspondientes
  async function save() {
    setSaving(true);
    try {
...
export default function ProfessionalManager({ onChanged, professionals }) {
  ...
  // Crear o registrar create aplicando las reglas de negocio y persistencia correspondientes
