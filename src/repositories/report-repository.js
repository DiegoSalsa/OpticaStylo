...
export async function getSalesReportData({ fromDate, origin, status, toDate }) {
  ...
  const [summary, statuses, methods, daily, products] = await Promise.all([
    executeQuery(
      ...
      COALESCE(SUM(discount_cents) FILTER (WHERE status <> 'CANCELLED'), 0) AS discount_cents,
      COALESCE(SUM(shipping_fee_cents) FILTER (WHERE status <> 'CANCELLED'), 0) AS shipping_cents,
      COALESCE((SELECT SUM(sale_payments.amount_cents) FROM sale_payments
        ...
        WHERE paid_sales.created_at >= $1 AND paid_sales.created_at < $2
          ...
          AND paid_sales.status <> 'CANCELLED'), 0) AS paid_cents
    ...
    executeQuery(
      ...
      FROM sales WHERE ${filter} AND status <> 'CANCELLED'
