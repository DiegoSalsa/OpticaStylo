...
export const SALE_STATUSES = Object.freeze([
  "QUOTATION", "PENDING", "PAID", "IN_PREPARATION", "READY", "DELIVERED", "CANCELLED",
]);
export const PAYMENT_METHODS = Object.freeze([
  "CASH", "BANK_TRANSFER", "MERCADO_PAGO", "TRANSBANK", "GETNET",
]);
...
function items(value) {
  ...
  const normalized = value.map((item, index) => {
    ...
    if (!Number.isInteger(item.quantity) || item.quantity < 1 || item.quantity > 1000) {
      fail(`La cantidad del producto en la posición ${index + 1} no es válida.`);
    }
    return {
...
export function validateSaleDraftInput(input) {
  ...
  return {
    ...
    prescriptionId: optionalId(input.prescriptionId, "receta"),
  };
}
...
export function validateSaleStatusInput(input) {
  ...
  if (!SALE_STATUSES.includes(status) || ["QUOTATION", "PENDING", "PAID"].includes(status)) {
    fail("El estado solicitado no puede asignarse manualmente.");
  }
  let cancellationReason = null;
  if (status === "CANCELLED") {
    cancellationReason = typeof input.cancellationReason === "string"
...
export function validateSaleListQuery(searchParams) {
  ...
  if (status && !SALE_STATUSES.includes(status)) fail("El estado no es válido.");
